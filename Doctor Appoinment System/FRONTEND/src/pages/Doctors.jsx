...
export default function Doctors() {
  ...
  // ✅ Filter doctors based on URL param (case-insensitive)
  const applyFilter = () => {
    if (speciality) {
      setFilterDoc(
        doctors.filter(
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
        ...
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter?'':'sm-flex hidden'}`}>
          <p
        ...
        {/* Doctors List */}
        <div className="grid grid-cols-auto gap-4 gap-y-6 w-full">
          {filterDoc.map((item) => (
            <div
              key={item._id}
